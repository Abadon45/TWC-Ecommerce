...
function submitForm() {
    ...
    // The visible phone input is formatted for the customer. Persist the
    // normalized value in the field consumed by the checkout view.
    let normalizedPhone = phone.replace(/\s+/g, "");
    if (normalizedPhone.length === 10 && /^[98]/.test(normalizedPhone)) {
        normalizedPhone = "0" + normalizedPhone;
    }
    $('#id_mobile_full, #id_full_mobile').val(normalizedPhone);
